...
El algoritmo escrito en Optuina permite paralelizar el proceso de optimización de forma asincrónica; esto se lograba utilizando un método llamado \textit{mentiroso constante} (\textit{constant liar}) \cite{NIPS2011_86e8f7ab}. Consiste en que al proponer un valor $\textbf{x}^*$, se asigne temporalmente una evaluación falsa igual al promedio de los valores de $y$ medidos, que luego se actualizará al valor real al momento de finalizar la evaluación. Este proceso puede hacer la optimización menos eficiente con respecto al número de iteracionies, pero más rapida a fin de cuentas ya que se pueden realizar varias iteraciones a la vez.
